...
export default function TodoList() {
    ...
    const saveTasks = async (tasksToSave) => {
        try {
            if (typeof window !== 'undefined' && window.storage) {
                await window.storage.set('todo-tasks', JSON.stringify(tasksToSave));
            } else {
                                            ...
                                            {task.subtasks.map(sub => (
                                                <div key={sub.id} className="flex items-center gap-3 group/sub">
                                                    ...
                                                    <button 
                                                        ...
                                                        className="opacity-100 sm:opacity-0 sm:group-hover/sub:opacity-100 p-1 text-zinc-400 hover:text-red-500"
                                                    >
                                                        <Trash2 size={14} />
                                                    </button>
                                                </div>
